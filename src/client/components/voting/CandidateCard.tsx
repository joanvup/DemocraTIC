...
export function CandidateCard({ candidate, onSelect, isSelected }: CandidateCardProps) {
  return (
    <div
      ...
      className={`group relative bg-white rounded-2xl p-5 border-2 transition-all cursor-pointer flex flex-col justify-between shadow-sm hover:shadow-xl ${
        isSelected
          ? 'border-emerald-500 ring-4 ring-emerald-500/20 bg-emerald-50/20 -translate-y-1'
          : 'border-slate-200 hover:border-sky-400 hover:-translate-y-1'
      }`}
    >
      {/* Badge Número de Lista */}
      <div className="absolute top-4 left-4 z-10">
        <span className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-black bg-sky-900 text-white shadow-md">
      ...
      {/* Foto del Candidato */}
      <div className="mt-8 mb-4 relative aspect-[4/3] rounded-xl overflow-hidden bg-slate-100 border border-slate-200">
        {candidate.photo_url ? (
          <img
      ...
      <div className="space-y-2 flex-grow">
        ...
        {candidate.slogan && (
          <p className="text-sm font-medium text-slate-600 italic bg-slate-50 p-2.5 rounded-lg border border-slate-100">
            {candidate.slogan}
          </p>
        )}
        ...
        <button
          ...
          className={`w-full py-3.5 px-4 rounded-xl font-bold text-base transition-colors flex items-center justify-center gap-2 ${
            isSelected
              ...
              : 'bg-slate-900 group-hover:bg-sky-700 text-white shadow-md'
          }`}
        >
...
export function BlankVoteCard({
  ...
  return (
    <div
      ...
      className={`group relative bg-white rounded-2xl p-5 border-2 transition-all cursor-pointer flex flex-col justify-between shadow-sm hover:shadow-xl ${
        isSelected
          ? 'border-slate-800 ring-4 ring-slate-800/20 bg-slate-100 -translate-y-1'
          : 'border-dashed border-slate-300 hover:border-slate-500 hover:-translate-y-1'
      }`}
    >
      <div className="text-center mt-6 mb-4">
        <div className="w-24 h-24 mx-auto bg-slate-100 rounded-full flex items-center justify-center border-2 border-slate-300 text-slate-400 group-hover:bg-slate-200 transition-colors">
        ...
        <button
          ...
          className={`w-full py-3.5 px-4 rounded-xl font-bold text-base transition-colors ${
            isSelected
              ...
              : 'bg-slate-200 group-hover:bg-slate-800 group-hover:text-white text-slate-700'
          }`}
        >
