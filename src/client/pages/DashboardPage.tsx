...
export function DashboardPage({
  ...
  // Modal de Reset de Censo
  const [isResetCensusModalOpen, setIsResetCensusModalOpen] = useState(false);
  const [resetMode, setResetMode] = useState<'EMPTY_CENSUS' | 'RESET_VOTES_ONLY'>('EMPTY_CENSUS');
  const [resetDeleteVotes, setResetDeleteVotes] = useState(true);
  ...
  // Ejecutar Reseteo del Censo / Puesta a Cero
  const handleExecuteResetCensus = async () => {
    if (resetConfirmWord.trim().toUpperCase() !== 'RESET') {
      setResetModalError('Debes escribir exactamente la palabra "RESET" para confirmar.');
      return;
    }
    ...
    try {
      ...
      if (resetMode === 'EMPTY_CENSUS') {
        const res = await adminApi.resetCensus(resetDeleteVotes, selectedElectionId);
        if (res.success) {
          setResetModalSuccess(res.message);
          await loadStudents();
          if (selectedElectionId) {
            await loadElectionData(selectedElectionId);
          }
          if (activeTab === 'AUDIT') {
            loadAuditLogs();
            loadMachineAudit();
          ...
          setTimeout(() => {
            setIsResetCensusModalOpen(false);
            setResetModalSuccess(null);
            setResetConfirmWord('');
          }, 2000);
        }
      } else {
        const res = await adminApi.resetVotingStatus(selectedElectionId);
        if (res.success) {
          setResetModalSuccess(res.message);
          await loadStudents();
          if (selectedElectionId) {
      ...
      const msg = err instanceof Error ? err.message : 'Error al procesar el reseteo';
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 flex flex-col">
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {(user?.role === 'SUPERADMIN' || user?.role === 'ADMIN_ELECTORAL') && (
                  <button
                    id="btn_reset_census_open"
                    type="button"
                    onClick={() => {
                      setIsResetCensusModalOpen(true);
                      setResetMode('EMPTY_CENSUS');
                      setResetDeleteVotes(true);
                      setResetConfirmWord('');
                      setResetModalError(null);
                      setResetModalSuccess(null);
                    }}
                    className="px-3.5 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shadow-sm"
                    title="Vaciar censo o reiniciar jornada para simulacro"
                  >
                    <RotateCcw className="w-3.5 h-3.5 text-rose-600" />
                    <span>Resetear Censo</span>
                  </button>
                )}
                <button
                  id="btn_add_student_open"
      ...
      {/* Modal de Resetear Censo / Puesta a Cero */}
      {isResetCensusModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            {/* Cabecera del Modal */}
            <div className="flex items-start gap-3.5 mb-5 pb-4 border-b border-slate-100">
              <div className="w-11 h-11 rounded-2xl bg-rose-100 border border-rose-200 flex items-center justify-center text-rose-600 flex-shrink-0">
                <AlertTriangle className="w-6 h-6" />
              </div>
              <div className="flex-grow">
                <div className="flex items-center justify-between">
                  <h3 className="font-black text-slate-900 text-lg">Resetear Censo Electoral</h3>
                  <span className="text-[10px] uppercase font-black px-2 py-0.5 rounded-full bg-rose-100 text-rose-700 tracking-wider">
                    Zona Crítica
                  </span>
                </div>
                <p className="text-xs text-slate-500 mt-0.5">
                  Selecciona la operación que deseas ejecutar sobre el censo y las urnas.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3 mb-5">
              <label className="text-xs font-bold text-slate-700 block">Tipo de Operación:</label>

              {/* Opción 1: Vaciar Censo Completo */}
              <label
                className={`flex items-start gap-3 p-3.5 rounded-2xl border transition-all cursor-pointer ${
                  resetMode === 'EMPTY_CENSUS'
              ...
              >
                <input
                  ...
                  name="reset_mode"
                  value="EMPTY_CENSUS"
                  checked={resetMode === 'EMPTY_CENSUS'}
                  onChange={() => setResetMode('EMPTY_CENSUS')}
                  className="mt-1 text-rose-600 focus:ring-rose-500"
                />
                <div className="flex-grow">
                  <div className="font-bold text-xs text-slate-900 flex items-center gap-1.5">
                    <span>Vaciar Censo Completo (Eliminar Estudiantes)</span>
                    <span className="text-[10px] font-bold px-1.5 py-0.5 bg-rose-100 text-rose-700 rounded">
                      Irreversible
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 mt-1">
                    Elimina todos los estudiantes registrados actualmente en la base de datos ({students.length} estudiantes). Deja la plataforma limpia para importar una nueva lista desde Excel.
                  </p>
                  {resetMode === 'EMPTY_CENSUS' && (
                    <div className="mt-2.5 pt-2.5 border-t border-rose-200/60">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={resetDeleteVotes}
                          onChange={e => setResetDeleteVotes(e.target.checked)}
                          className="rounded text-rose-600 focus:ring-rose-500"
                        />
                        <span className="text-[11px] font-bold text-slate-700">
                          Purgar también los votos emitidos en la urna electrónica
                        </span>
                      </label>
                    </div>
                  )}
              ...
              {/* Opción 2: Puesta a Cero (Simulacro) */}
              <label
                className={`flex items-start gap-3 p-3.5 rounded-2xl border transition-all cursor-pointer ${
                  resetMode === 'RESET_VOTES_ONLY'
              ...
              >
                <input
                  type="radio"
                  name="reset_mode"
                  value="RESET_VOTES_ONLY"
                  checked={resetMode === 'RESET_VOTES_ONLY'}
                  onChange={() => setResetMode('RESET_VOTES_ONLY')}
                  className="mt-1 text-blue-600 focus:ring-blue-500"
                />
                <div className="flex-grow">
                  <div className="font-bold text-xs text-slate-900 flex items-center gap-1.5">
                    <span>Puesta a Cero de Votación (Simulacro)</span>
                    <span className="text-[10px] font-bold px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded">
                      Conserva Carnets
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 mt-1">
                    Mantiene intactos a los estudiantes y sus carnets QR, pero anula los votos emitidos, tokens consumidos y registros de estaciones, regresando a todos a estado <strong>PENDIENTE</strong>.
                  </p>
                </div>
              </label>
            </div>
            ...
            <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 mb-5">
              <label className="block text-xs font-bold text-slate-700 mb-1">
                Para confirmar la acción, escribe la palabra <span className="text-rose-600 font-mono font-black">RESET</span>:
              </label>
              <input
                id="reset_census_confirm_input"
                type="text"
                value={resetConfirmWord}
                onChange={e => setResetConfirmWord(e.target.value)}
                placeholder="Escribe RESET aquí"
                className="w-full uppercase font-mono tracking-widest text-center py-2 px-3 bg-white border border-slate-300 rounded-xl text-xs font-black focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20"
              />
            </div>
            ...
            <div className="flex gap-2.5">
              <button
                ...
                className="w-1/2 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-xs cursor-pointer transition-colors"
              >
                Cancelar
              </button>
              <button
                id="execute_reset_census_btn"
                type="button"
                onClick={handleExecuteResetCensus}
                disabled={resetModalLoading || resetConfirmWord.trim().toUpperCase() !== 'RESET'}
                className="w-1/2 py-2.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-md shadow-rose-600/20 transition-all"
              >
                {resetModalLoading ? (
                  <>
