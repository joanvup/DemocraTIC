              ...
              <div className="flex items-center gap-3">
                {currentElection?.status === 'OPEN' ? (
                  <button
                    ...
                    className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold transition-colors shadow"
                  >
                    Cerrar Elección
                  </button>
                ) : (
                  <button
                    ...
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-colors shadow"
                  >
                    Abrir Elección
                  </button>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {elections.map(elec => {
                ...
                return (
                  <div 
                    key={elec.id} 
                    className={`bg-white p-6 rounded-3xl border ${isSelected ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-200'} shadow-sm space-y-4 transition-all`}
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-slate-400">Año {elec.year}</span>
                          {isSelected && (
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                        ...
                        {elec.status === 'OPEN' ? '🟢 ABIERTA' : elec.status === 'CLOSED' ? '🔴 CERRADA' : elec.status}
                      </span>
                    </div>
                    ...
                    <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          title="Editar parámetros de la elección"
                        >
