...
import { detectDeviceMetadata, getOrInitStationId, setCustomStationName } from '../utils/deviceInfo.js';
import {
  BarChart3,
  Calendar,
  Camera,
...
export function VotingStationPage({
  ...
  // Identificación del Equipo / Terminal de Votación
  const [stationId, setStationId] = useState<string>(() => getOrInitStationId());
  const [isEditStationModalOpen, setIsEditStationModalOpen] = useState(false);
  const [customStationInput, setCustomStationInput] = useState(stationId);
  ...
  const handleConfirmVote = async () => {
    ...
    try {
      ...
      const res = await votingApi.castVote({
        ...
        station_id: meta.station_id || stationId,
    ...
    <div className={`min-h-screen flex flex-col justify-between select-none transition-colors duration-200 ${
      ...
      <footer className={`border-t px-6 py-3 text-center text-xs flex flex-col sm:flex-row items-center justify-between gap-2 transition-colors ${
        ...
        <div className="flex items-center gap-2">
          <span className="font-mono text-[11px] text-slate-400 flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800/80 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700">
            <Monitor className="w-3.5 h-3.5 text-sky-500" />
            <span>Terminal: <strong className="text-slate-700 dark:text-slate-200">{stationId}</strong></span>
          </span>
          <button
            type="button"
            onClick={() => {
            ...
            title="Nombrar o identificar este equipo de votación"
            className="text-[11px] font-semibold text-sky-600 dark:text-sky-400 hover:underline px-1.5 py-1 rounded hover:bg-sky-50 dark:hover:bg-sky-950/40 cursor-pointer flex items-center gap-1"
          >
            <Edit2 className="w-3 h-3" />
            <span className="hidden sm:inline">Identificar Equipo</span>
          </button>
        </div>
      </footer>

      {/* Modal para Identificar / Nombrar el Equipo */}
          ...
          <div className={`rounded-2xl max-w-md w-full p-6 border shadow-2xl ${
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2 text-sky-500">
                ...
                <h3 className="font-bold text-base text-slate-900 dark:text-white">Identificador del Equipo</h3>
              </div>
              <button
                type="button"
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 leading-relaxed">
              Asigna un nombre o número para esta máquina (ej. <em>SALA-1-PC-04</em> o <em>PORTATIL-03</em>). Quedará guardado en este equipo para auditar las votaciones registradas.
            </p>
            <div className="space-y-3">
              <div>
                <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block mb-1">
                  Nombre o Código de Terminal
                </label>
                <input
                  type="text"
                  value={customStationInput}
                  onChange={(e) => setCustomStationInput(e.target.value.toUpperCase())}
                  placeholder="Ej: SALA1-PC05"
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 uppercase font-bold"
                />
              </div>
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  type="button"
                  onClick={() => {
                    const newId = customStationInput.trim() || stationId;
                    setCustomStationName(newId);
                    setStationId(newId);
                    setIsEditStationModalOpen(false);
                  }}
                  className="px-5 py-2 rounded-xl text-xs font-bold bg-sky-600 hover:bg-sky-500 text-white cursor-pointer shadow-md"
                >
